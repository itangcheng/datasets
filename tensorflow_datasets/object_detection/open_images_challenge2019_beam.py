...
class CreateDetectionExampleFn(beam.DoFn):
  ...
  def __getstate__(self):
    return {
        "_image_labels_filepath": self._image_labels_filepath,
        "_box_labels_filepath": self._box_labels_filepath,
        "_hierarchy_filepath": self._hierarchy_filepath,
        "_classes_filepath": self._classes_filepath,
    }
...
def load_box_level_labels(filepath):
  ...
  with tf.io.gfile.GFile(filepath, "r") as csvfile:
    ...
    for row in reader:
      if len(row) == 7:
        image_id, label, xmin, xmax, ymin, ymax, is_group_of = row
      elif len(row) == 13:
        image_id, label = row[0], row[2]
        xmin, xmax, ymin, ymax = row[4:8]
        is_group_of = row[10]
      xmin, xmax, ymin, ymax = [float(x) for x in (xmin, xmax, ymin, ymax)]
      is_group_of = bool(int(is_group_of))
